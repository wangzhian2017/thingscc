...
class ThrowFruit(pygame.sprite.Sprite):
    ...
    def __init__(self, manager,window, image_path, speed, turn_angel, flag):
    ...
    def update(self):
        ...
        if self.rect.y >= self.height + self.rect.height:
            if self.flag != 5:
                self.manager.classic_miss += 1
            self.kill()
        self.rect.y -= self.v0 * self.fruit_t * math.sin(self.throw_angel / 100) - (self.manager.G *
                                                                                    self.fruit_t ** 2 / 10) / 2
